  ...
  <div class="h-screen overflow-hidden context">
    ...
    <div class="flex flex-row w-full h-full">
      <div class="flex flex-col items-center pt-5 bg-white">
        ...
        <NuxtLink :to="{ path: 'project', query: { title: projectName }}" class="text-md text-black font-bold text-opacity-25">Back to {{projectName}}</NuxtLink>
          ...
          <div class="bg-white" aria-label="Sidebar">
            <div class="pb-6 mb-2 space-y-3">
              <div class="flex justify-center pb-4">
                <span class="text-lg text-black font-bold text-opacity-50">Emails</span>
              </div>            
              ...
              <button @click="displayJsonList = !displayJsonList" class="w-64 h-12 relative inline-block cursor-pointer bg-gradient-to-r from-ap-purple to-green-400 hover:from-ap-purple hover:to-blue-500 text-white text-center font-bold p-2 mx-2 rounded-md focus:outline-none">
                Option 2
              </button>
              <div v-if="displayJsonList" class="">
                <div class="rounded-md bg-white shadow-xs">
                  <div class="pt-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                    <p class="text-sm leading-5 font-medium text-center py-2">/static</p>
                    <div class="border-t border-2 border-gray-200"></div>
                    <span v-for="(json, index) in jsonList.keys()" :key="index" @click="loadJsonFromList(json)" class="block px-4 py-2 text-sm text-center leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900 cursor-pointer" role="menuitem">{{ json }}</span>
                    <span class="block px-4 py-2 mb-0 text-sm leading-5 text-gray-700 hover:bg-red-400 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900 cursor-pointer text-center font-bold" @click="displayJsonList = !displayJsonList">Close</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="flex flex-col parent2">
          <div class="child2 space-y-8">
                ...
                <div v-for="(entry, entryIndex) in orderedEntries" :key="entryIndex" class="ml-2">
                  <div v-if="entry.type == 'delimiter'" class="mb-2 flex justify-start" :class="{'border border-blue-800 rounded-md p-1': orderedEntries[entryIndex].outline}">
                    ...
                    <a v-if="!orderedEntries[entryIndex].editing" class="cursor-pointer" v-scroll-to="{
                      ...
                      <!-- <span v-for="(word, index) in entry.text.split(/(?=[.\s]|\b)/)" :key="index">
                        <span v-if="keywords.includes(word)" class="text-blue-700 hover:text-red-600" :ref="word" @mouseover="highlightKeyword(word)" @mouseout="highlightKeyword(word)">{{word}}</span>
            ...
            <div>
              ...
              <div v-if="keywords.length && orderedEntries.length" class="flex flex-row flex-wrap my-2 mr-16 child2">
                <div v-for="(keyword, index) in keywords" :key="index" class="ml-2">
                  <!-- <div class="border border-blue-700 text-blue-700 px-3 py-1 rounded-md border-1 cursor-pointer my-1" @mouseover="highlightKeyword(keyword)" @mouseout="highlightKeyword(keyword)">
                    {{ keyword }}
                  </div> -->
          ...
          >
            <template v-for="(entry, entryIndex) in orderedEntries" class="">
              ...
              <div v-else :id="'highlight-'+ entryIndex" :ref="'highlight-' + entryIndex" :key="entryIndex" class="pt-2 pb-4 mx-5 cursor-pointer draggable-entry group" data-type="delimiter">
                ...
                <!-- <button @click="markKeyword">lalalala</button> -->
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      jsonList: {},
      displayJsonList: false,
      oldEntry: "",
      keywords: ["Katharina", "Farbod", "Uwe", "Ashish", "Tomas", "Loujith", "DCIC A", "CR", "DIBS", "Accenture", "CR", "Core 2.0", "One Strike", "GMP", "ACI"]
    }
  },
  mounted() {
    this.jsonList = require.context('../static', true, /([a-zA-Z0-9\s_\\.\-\(\):])+(.json)$/i)
  },
  updated() {
  ...
  methods: {
    ...
    loadJsonFromList(name) {
      this.entries = []
      this.orderedEntries = []
      this.attendees = []

      name = name.slice(2)
      this.data = require('../static/' + name)
      this.displayJsonList = !this.displayJsonList

      if(!(this.data).hasOwnProperty('sections')) {
        alert('Sorry, invalid format for meeting file.')
        return
      }
