<template>
  <div class="h-screen overflow-hidden context">
    ...
    <div class="flex flex-row w-full h-full">
          ...
          <div class="bg-white" aria-label="Sidebar">
            <div class="pb-6 mb-2 space-y-3">
              ...
              <div class="my-2 flex flex-row items-center">
                ...
                <label for="upload-json"
                  class="flex items-center justify-center w-64 h-12 cursor-pointer bg-gradient-to-r from-ap-purple to-green-400 hover:from-ap-purple hover:to-blue-500 text-white text-center font-bold p-2 mx-2 rounded-md">
                  Upload meeting file
                </label>
              </div>
              <button @click="displayMeetings = !displayMeetings"
                class="w-64 h-12 relative inline-block cursor-pointer bg-gradient-to-r from-ap-purple to-green-400 hover:from-ap-purple hover:to-blue-500 text-white text-center font-bold p-2 mx-2 rounded-md focus:outline-none">
                Load existing meeting
              </button>
              <div class="rounded-md bg-white shadow-xs">
                <div role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                  <div v-if="displayMeetings && currentProject">
                    <span v-for="(meeting, index) in currentProject.meetings" :key="index" @click="changeMeeting(index)"
                      class="block px-4 py-2 text-sm text-center leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900 cursor-pointer"
                      role="menuitem">{{ meeting.date }}</span>
                    <span
                      class="block px-4 py-2 mb-0 text-sm leading-5 text-gray-700 hover:bg-red-400 hover:text-gray-900 focus:outline-none cursor-pointer text-center font-bold"
                      @click="displayMeetings = !displayMeetings">Close</span>
                  </div>
                </div>
              </div>
            </div>
            <div v-if="sections.length" class="py-6 my-2 space-y-3">
              ...
              <button
                class="w-64 h-12 cursor-pointer transition duration-500 ease-in-out border border-transparent text-white text-center font-bold p-2 mx-2 rounded-md bg-blue-600 hover:bg-white hover:text-blue-800 hover:border-blue-800 focus:outline-none">
                Send minute to participants
              </button>
              <button
                class="flex flex-row py-8 justify-center items-center w-64 h-12 transition duration-500 ease-in-out font-bold px-2 ml-2 rounded-md border focus:outline-none hover:text-blue-700 hover:bg-white border-blue-700 bg-blue-600 text-white focus:ring-blue-800">
            ...
            <div v-if="sections.length" class="py-6 my-2 space-y-3">
              ...
              <div class="flex flex-col items-center space-y-1">
                ...
                <input type="number" placeholder="Shots" ref="shots" min="0" value="3"
                  class="p-2 font-bold focus:ring-blue-800 focus:border-blue-600 border-blue-600 rounded-md w-24">
              </div>
            </div>
          </div>
        </div>
      ...
      <div
        class="md:w-3/5 border-transparent border-2 p-4 rounded-tl bg-gray-50 border-t border-r border-gray-200 pt-8">
        <div class="flex flex-col parent2">
          <div class="child2 space-y-8">
              ...
              <div v-if="attendees.length" class="my-2">
                <div class="flex flex-row">
                  <div v-for="(attendee, attendeeIndex) in attendees" :key="attendeeIndex">
                    <div class="rounded-md mx-2 px-3 py-1 text-white text-center text-base"
                      :class="'bg-' + attendeesColors[attendee]">
            ...
            <div>
              ...
              <div v-if="orderedEntries.length" class="flex flex-col my-2 mr-16 child2 context">
                <div v-for="(entry, entryIndex) in orderedEntries" :key="entryIndex" class="ml-2">
                  <div v-if="entry.type == 'delimiter'" class="mb-2 flex justify-start"
                    :class="{ 'border border-blue-800 rounded-md p-1': orderedEntries[entryIndex].outline }">
                    <div class="mr-2 flex flex-row space-x-2 items-center"
                      v-if="!orderedEntries[entryIndex].editing && orderedEntries[entryIndex].text != ''">
                      <input type="checkbox" class="focus:ring-white" checked :ref="'check-' + entryIndex">
                      <svg class="inline cursor-pointer mb-1 w-5 transform hover:scale-110"
                        @click="editHighlight(entryIndex)" viewBox="0 0 56 56" xmlns="http://www.w3.org/2000/svg">
                        <g id="Page-1" fill="none" fill-rule="evenodd">
                          <g id="028---Coloured-Pencil" fill="rgb(0,0,0)" fill-rule="nonzero">
                            <path id="Shape"
                              d="m37.381 3.062c1.2207502-.58548714 2.6762453-.35630038 3.658.576.9390562.98619475 1.1703464 2.45036916.581 3.678l-31.42 31.419c-.32511253-2.1939403-2.04185317-3.9200067-4.234-4.257z" />
                            <path id="Shape"
                              d="m47.3 12.94-31.441 31.436c-.3320279-2.1870828-2.0488389-3.9034885-4.236-4.235l31.437-31.436c1.2155936-.6078953 2.6837538-.37013899 3.6453629.59033615.9616092.96047515 1.2010977 2.42835375.5946371 3.64466385z" />
                            <path id="Shape"
                              d="m1.171 55.985c-.32329334.055683-.65333752-.0507433-.88320188-.2847979-.22986437-.2340546-.33031168-.5659673-.26879812-.8882021l.681-4.433c1.18520301.3028582 2.2657494.9222885 3.126 1.792.87107224.8608776 1.49155769 1.9424926 1.795 3.129z" />
                            <path id="Shape"
                              d="m17.233 53.5-9.624 1.49c-.37308438-1.6061263-1.19340297-3.0737546-2.366-4.233-1.15921198-1.1726409-2.62685381-1.9929671-4.233-2.366l1.49-9.624c.15163785-.7975088.58019655-1.5157873 1.21-2.028 1.2198302-.5849799 2.67419993-.3557824 3.655.576.95428456 1.0206965 1.16809197 2.5293375.535 3.775-.20594307.3870332-.13577323.8630791.17308318 1.1742301.30885641.3111511.78436986.3848426 1.17291682.1817699 1.2462801-.6355962 2.7570717-.4233659 3.78.531.9537783 1.0206046 1.1671864 2.5288487.534 3.774-.2061784.3874047-.1356804.8639667.1738309 1.1750852.3095112.3111185.7857017.3840839 1.1741691.1799148 1.2456658-.6352592 2.7557086-.4230218 3.778.531.9341031.9830018 1.1626345 2.441382.574 3.663-.5135628.6258809-1.2312601 1.0507633-2.027 1.2z" />
                            <path id="Shape"
                              d="m20.1 47.222c-.7608792-.7733981-1.7555878-1.2742673-2.83-1.425l31.414-31.417c1.2171322-.6251157 2.698435-.3947368 3.6682285.5704994.9697936.9652362 1.2071472 2.4454374.5877715 3.6655006l-31.419 31.418c-.1506789-1.0683668-.6502557-2.0569734-1.421-2.812z" />
                            <path id="Shape"
                              d="m55.194 16.346c-.1537051-1.0639379-.6530187-2.0478072-1.421-2.8-1.1014064-1.1115871-2.6580246-1.6462355-4.21-1.446.1921248-1.5521143-.3421076-3.10611899-1.4479943-4.21200568-1.1058867-1.10588668-2.6598914-1.6401191-4.2120057-1.44799432.1997386-1.55289907-.3356228-3.11017969-1.448-4.212-.751672-.76834259-1.7359046-1.26713761-2.8-1.419 1.1799454-1.09380356 3.012669-1.06203753 4.154.072l11.312 11.311c1.1430927 1.1373382 1.1754253 2.9772003.073 4.154z" />
                          </g>
                        </g>
                      </svg>
                      <svg class="inline cursor-pointer mb-1 w-5 transform hover:scale-110"
                        @click="deleteDelimiter(entryIndex)" viewBox="-57 0 512 512" xmlns="http://www.w3.org/2000/svg">
                        <path
                    ...
                    <textarea v-else rows="3" cols="30" class="w-full my-2" v-model="orderedEntries[entryIndex].text"
                      @blur='resetEditHighlight(entryIndex)' v-on:keyup.enter="resetEditHighlight(entryIndex)"
                      ref="editHighlight"></textarea>
                  </div>
                </div>
              </div>
              <div v-else>
            ...
            <div>
              ...
              <div v-if="meeting && meeting.keywords.length && orderedEntries.length"
                class="flex flex-row flex-wrap my-2 mr-16 child2">
                <div v-for="(keyword, index) in meeting.keywords" :key="index" class="ml-2">
                  <div class="border border-blue-700 text-blue-700 px-3 py-1 rounded-md border-1 cursor-pointer my-1"
                    @mouseover="markKeyword(keyword)">
                    {{ keyword }}
                  </div>
                </div>
              </div>
      ...
      <div class="md:w-2/5 p-4 border-t border-gray-200 border-transparent rounded-tr shadow-xl pt-8" ref="transcript">
        ...
        <div v-if="createSections" class="inline-block mr-2">
          <input type="number" placeholder="Granularity" ref="granularity" min="1"
            class="ml-2 p-2 font-bold focus:ring-blue-600 focus:border-blue-600 border-blue-600 rounded-md">
          <button type="button" @click="setGranularity"
            class="transition duration-500 ease-in-out p-2 border border-transparent text-base text-white font-bold hover:text-blue-600 font-medium rounded-md bg-blue-600 hover:bg-white hover:border-blue-600 focus:outline-none">
            Submit
          </button>
        </div>
        <button v-if="sections.length" @click="toggleCreateSections"
        ...
        <div class="flex flex-col my-2 pl-2 parent context">
          <draggable class="child" draggable=".draggable-entry" v-model="orderedEntries" :move="onMoveCallback"
            @change="clearDelimiterHighlights($event)" id="transcript">
            <template v-for="(entry, entryIndex) in orderedEntries" class="">
              <div v-if="entry.type != 'delimiter'" class="my-4 draggable-entry group" :key="entryIndex"
                data-type="entry">
                <div class="flex flex-row items-center justify-start space-x-2">
                  <div v-if="entry.speaker"
                    class="border-2 border-transparent rounded-full h-8 w-8 flex items-center justify-center text-white"
                    :class="'bg-' + attendeesColors[entry.speaker]">
                ...
                <p v-if="!orderedEntries[entryIndex].hasOwnProperty('editing')" class="mt-1 ml-10 text-sm">
                  <span>{{ entry.text }}</span>
                  <svg class="inline cursor-pointer mb-1 w-4 transform hover:scale-110" @click="editEntry(entryIndex)"
                ...
                <textarea v-else rows="3" cols="30" class="w-full my-2" v-model="orderedEntries[entryIndex].text"
                  @blur='resetEditEntry(entryIndex)' v-on:keyup.enter="resetEditEntry(entryIndex)"
                  ref="editEntry"></textarea>
                <span
                  v-if="(entryIndex + 1) != orderedEntries.length && !orderedEntries[entryIndex + 1].hasOwnProperty('type')"
                  class="invisible group-hover:visible text-black text-opacity-50 cursor-pointer"
                  @click="addDelimiter(entryIndex)">Add delimiter</span>
              </div>
              <div v-else :id="'highlight-' + entryIndex" :ref="'highlight-' + entryIndex" :key="entryIndex"
                class="pt-2 pb-4 mx-5 cursor-pointer draggable-entry group" data-type="delimiter">
                <div class="flex flex-row justify-between">
                  <span v-if="!entry.text"
                    class="invisible group-hover:visible text-black text-opacity-50 cursor-pointer"
                    @click="editHighlight(entryIndex)">Add text</span>
                  <span class="invisible group-hover:visible text-black text-opacity-50 cursor-pointer"
                    @click="deleteDelimiter(entryIndex)">Delete delimiter</span>
                </div>
                <div class="h-1 bg-black rounded-full w-full flex flex-row justify-center items-center">
                  <div v-if="!entry.text">
                    <div class="flex-row justify-center">
                      <div class="rounded-full border border-black transform hover:scale-110">
                        <svg v-if="!entry.loading" @click="sectionsToApi(entryIndex)"
                          class="h-6 w-6 rounded-full p-1 bg-white" xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 24 24">
                          <path d="M24 10h-10v-10h-4v10h-10v4h10v10h4v-10h10z" />
                        </svg>
                        <svg v-if="entry.loading" class="h-6 w-6 rounded-full bg-white"
                          xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                          style="display: block; shape-rendering: auto;" viewBox="0 0 100 100"
                          preserveAspectRatio="xMidYMid">
                          <circle cx="50" cy="50" fill="none" stroke="black" stroke-width="10" r="35"
                  ...
                  <div v-else class="rounded-full border border-black transform hover:scale-110">
                    <svg v-if="!entry.loading" class="h-6 w-6 rounded-full p-1 bg-white"
                      @click="sectionsToApi(entryIndex)" xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px"
                      viewBox="0 0 489.645 489.645" style="enable-background:new 0 0 489.645 489.645;"
                      xml:space="preserve">
                      <g>
                        <path
                    ...
                    <svg v-if="entry.loading" class="h-6 w-6 rounded-full bg-white" xmlns="http://www.w3.org/2000/svg"
